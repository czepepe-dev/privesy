...
async function nactiVsechnyProdukty() {
  ...
  for (const file of seznam) {
    try {
      const resp = await fetch(`/${PRODUCT_PATH}/${encodeURIComponent(file)}?t=${Date.now()}`);
      if (!resp.ok) continue;
  ...
  // ZÁSADNÍ: vždy nejnovější přívěs jako první.
  // Datum přidání zapisuje administrace automaticky při vytvoření produktu.
  produkty.sort((a, b) => {
    const da = Date.parse(a.datumPridani || "");
    const db = Date.parse(b.datumPridani || "");
